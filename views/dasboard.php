...
try {
    ...
    // Stats dari riwayat_obat hari ini
    $stmt = $conn->prepare("
        SELECT 
            COUNT(*) as total,
            SUM(CASE WHEN status = 'Sudah Diminum' THEN 1 ELSE 0 END) as diminum
        FROM riwayat_obat 
        WHERE user_id = ? AND DATE(waktu_jadwal) = CURDATE()
    ");
    $stmt->bind_param("i", $user_id);
    $stmt->execute();
    ...
    $jumlah_terlewat = $total_jadwal - $jumlah_diminum;
    ...
    <div class="card" style="padding:var(--space-4)">
        ...
        <?php if (count($jadwal_today) > 0): ?>
            ...
            foreach ($jadwal_today as $item):
                ...
                $is_completed = (bool)$item['status_hari_ini'];
                $jam_jadwal = DateTime::createFromFormat('H:i:s', $item['jam_minum']);
                $is_missed = !$is_completed && $jam_jadwal && $jam_jadwal <= $sekarang;
                ...
                <?php if (!$is_completed): ?>
                    <form action="../proses/prosesKonsumsi.php?aksi=catat" method="POST" style="margin:0">
                        ...
                        <button type="submit" class="btn btn-sm" style="background:<?= $is_missed ? 'var(--secondary)' : 'var(--primary-container)' ?>;color:<?= $is_missed ? 'var(--on-secondary)' : 'var(--on-primary-fixed)' ?>;border-radius:var(--radius-full)">
                            <span class="material-symbols-sharp" style="font-size:14px"><?= $is_missed ? 'refresh' : 'check' ?></span> <?= $is_missed ? 'Catat' : 'Minum' ?>
                        </button>
                    </form>
                <?php else: ?>
